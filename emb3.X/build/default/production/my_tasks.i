...
void debounce(void) {
    ...
    if(PORTCbits.RC1 == 1 && !LOCK_MODE && SYSTEM_RUNNING) {
        if(debounceS2 == 0) {
            if(GLOBAL_TEMPERATURE < 50){
                GLOBAL_TEMPERATURE ++;
            }
        }
        debounceS2 = 100;
    ...
    if(PORTCbits.RC2 == 1 && !LOCK_MODE && SYSTEM_RUNNING) {
        if(debounceS3 == 0) {
            if(GLOBAL_TEMPERATURE > 10){
                GLOBAL_TEMPERATURE --;
            }
        }
        debounceS3 = 100;
    ...
    if(PORTCbits.RC3 == 1 && LOCK_MODE && SYSTEM_RUNNING) {
        if(debounceS4 == 0) {
            LOCK_MODE = 0;
            FLA_ERROR = 0;
            IGN_ERROR = 0;
...
void blink(void) {
    if(SYSTEM_RUNNING && !LOCK_MODE) PORTCbits.RC4 = !PORTCbits.RC4;
}

void led_set(void) {
    PORTBbits.RB7 = LOCK_MODE && SYSTEM_RUNNING;
    PORTBbits.RB6 = FLA_ERROR && SYSTEM_RUNNING;
    PORTCbits.RC7 = IGN_ERROR && SYSTEM_RUNNING;
    PORTCbits.RC6 = PORTCbits.RC6 && SYSTEM_RUNNING && (!LOCK_MODE);
    PORTCbits.RC5 = PORTCbits.RC5 && SYSTEM_RUNNING && (!LOCK_MODE);
    PORTCbits.RC4 = PORTCbits.RC4 && SYSTEM_RUNNING && (!LOCK_MODE);
}
